...
namespace Plushki\Identity\Platform;

/**
 * Config is the resolved runtime configuration for identity.
 *
 ...
 *  - jwtPrivateKeyPath: PEM RSA key. Empty + env=dev -> Kernel generates an
 *    ephemeral key (restart invalidates issued access tokens).
 *  - bootstrapAdminEmail: a user granted the "admin" role on every startup if
 *    they exist. Empty (default) = do nothing.
 */
final class Config
{
    public function __construct(
        public readonly string $service = 'identity',
        ...
        public readonly string $jwtPrivateKeyPath = '',
        public readonly string $jwtKeyId = 'k1',
        public readonly string $jwtIssuer = 'https://identity.plushki',
        public readonly string $bootstrapAdminEmail = '',
    ) {
    }
    ...
    public static function load(?string $configYamlPath = null): self
    {
        ...
        $values = [
            ...
            'jwt_private_key_path' => $d->jwtPrivateKeyPath,
            'jwt_key_id' => $d->jwtKeyId,
            'jwt_issuer' => $d->jwtIssuer,
            'bootstrap_admin_email' => $d->bootstrapAdminEmail,
        ];
        ...
        $cfg = new self(
            ...
            jwtPrivateKeyPath: $values['jwt_private_key_path'],
            jwtKeyId: $values['jwt_key_id'],
            jwtIssuer: $values['jwt_issuer'],
            bootstrapAdminEmail: $values['bootstrap_admin_email'],
        );
