...
/**
 * Migrator applies the goose-annotated *.sql files under migrations/. The
 * migration files keep the
 * `-- +goose Up / -- +goose Down / StatementBegin / StatementEnd` markers; we
 * parse out the Up section and run it.
 *
 * Applied files are tracked in plushki_migrations (one row per filename), so a
 * second run is a no-op: only pending up-migrations run on startup.
 */
